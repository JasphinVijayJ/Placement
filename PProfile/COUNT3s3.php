...
{ 
    $Csem = $_POST['csem'];
    $query = "SELECT count(*) FROM basicdetails WHERE `Approve`='1' AND Sem='$Csem'";
    $result = mysqli_query($connect, $query);
    $data = mysqli_fetch_assoc($result);

    echo "<br><h3>Students in Semester '$Csem': " . $data['count(*)'] . "</h3>";

    $sql = mysqli_query($connect, "SELECT * FROM basicdetails WHERE `Approve`='1' AND Sem='$Csem' ORDER BY Branch");

    while($row = mysqli_fetch_assoc($sql)) {
        // Your code to display the student details
	            print "<tr>"; 	
	echo '<td>'.$row['Sem'].'</td>';	
